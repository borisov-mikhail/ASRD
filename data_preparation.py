...
def plot_graph(index_of_sample, x, y):
    data = submissions_data[indexes[index_of_sample]+1:indexes[index_of_sample+1]]
    sns.set_style("ticks", {'xtick.color': '.0', 'ytick.color': '.0'})
    g = sns.lmplot(x=x, y=y, data=data, hue='0_1', height=3, aspect=4, legend=0, fit_reg=False,
                   scatter_kws={"s": 50})
